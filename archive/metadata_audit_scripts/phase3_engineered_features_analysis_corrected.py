...
"""
Phase 3: Engineered Features Investigation (Corrected)
...
def load_data():
    ...
    # Load the sample processed data which contains ALL engineered features
    processed_sample = pd.read_csv(RESULTS_DIR / 'analysis' / 'processed_data_sample.csv')
    
    print(f"✅ Loaded sample: {len(processed_sample)} rows with {len(processed_sample.columns)} columns")
...
def analyze_patient_relative_features(df):
    ...
    for base_col in RELATIVE_FEATURE_COLS[:3]:  # Analyze first 3 key features
    ...
    for i, feat in enumerate(key_features):
        if feat in df.columns:
            ...
            axes[i].hist(benign, bins=30, alpha=0.5, label='Benign', density=True)
            if len(malignant) > 0:
                axes[i].hist(malignant, bins=30, alpha=0.5, label='Malignant', density=True)
    ...
    if 'patient_lesion_count' in df.columns:
        axes[3].hist(df['patient_lesion_count'], bins=30, alpha=0.7, edgecolor='black')
    ...
    if 'patient_lof' in df.columns:
        ...
        if len(lof_data) > 0:
            axes[4].hist(lof_data, bins=30, alpha=0.7, edgecolor='black')
            ...
            if len(lof_malignant) > 0:
                axes[5].hist(lof_benign, bins=20, alpha=0.5, label='Benign', density=True)
                axes[5].hist(lof_malignant, bins=20, alpha=0.5, label='Malignant', density=True)
            ...
            axes[4].text(0.5, 0.5, 'No valid LOF data', 
                        transform=axes[4].transAxes, ha='center', va='center')
            axes[4].set_title('Distribution of LOF Scores')
            axes[5].text(0.5, 0.5, 'No valid LOF data', 
                        transform=axes[5].transAxes, ha='center', va='center')
            axes[5].set_title('LOF Distribution by Target')
...
def analyze_lof(df):
    ...
    if valid_count > 0:
        ...
        results['lof_by_target'] = lof_by_target.to_dict()
...
def analyze_vision_predictions(df):
    ...
    for i, col in enumerate(available_cols):
        ...
        axes[1, i].hist(benign, bins=20, alpha=0.5, label='Benign', density=True)
        if len(malignant) > 0:
            axes[1, i].hist(malignant, bins=20, alpha=0.5, label='Malignant', density=True)
...
def main():
    ...
    print("🚀 Starting Phase 3: Engineered Features Investigation (Corrected)")
    
    # Create plots directory if it doesn't exist
    PLOTS_DIR.mkdir(exist_ok=True)
    
    # Load data - use sample data which has all engineered features
    df = load_data()
    ...
    import json
    with open(RESULTS_DIR / 'analysis' / 'phase3_results_corrected.json', 'w') as f:
        json.dump(results, f, indent=2, default=str)
    
    print(f"\n✅ Phase 3 analysis complete!")
    print(f"📊 Results saved to: {RESULTS_DIR / 'analysis' / 'phase3_results_corrected.json'}")
