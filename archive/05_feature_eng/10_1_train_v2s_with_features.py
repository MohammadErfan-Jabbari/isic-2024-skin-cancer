"""
EfficientNetV2-S Hybrid with Feature Engineering
Enhances metadata with clinical domain knowledge

Usage:
    python train_v2s_with_features.py --gpu 0
    python train_v2s_with_features.py --gpu 1 --epochs 30
"""
...
def parse_args():
    ...
    parser.add_argument('--gpu', type=int, default=0, help='GPU ID (0-3)')
...
def engineer_features(df):
    """
    Add clinically-relevant features for melanoma detection.
    Based on ABCDE rule: Asymmetry, Border, Color, Diameter, Evolution
    """
    df = df.copy()
    ...
    # SHAPE FEATURES (Border irregularity)
    ...
    def __init__(self, hdf5_path, metadata_df, transform=None, is_test=False):
        ...
        if not is_test and 'target' in self.metadata.columns:
            print(f"  ✓ {len(self.metadata)} samples, "
                  f"pos={self.metadata['target'].sum()}")
...
def main():
    ...
    print("="*70)
    print("EFFICIENTNETV2-S + FEATURE ENGINEERING")
    print("="*70)
    ...
    print(f"NEW: Enhanced metadata with 15+ engineered features")
    print("="*70 + "\n")
    ...
    print(f"✓ Metadata dimension: {metadata_dim} (was ~50, now ~80-90)\n")
    ...
    results_dir = Path('results') / f'v2s_features_{timestamp}'
    ...
    with open(results_dir / 'training_results.pkl', 'wb') as f:
        pickle.dump({
            'model': 'EfficientNetV2-S + Features',
            'best_auc': best_auc,
            'history': history,
    ...
    submission.to_csv(results_dir / 'submission_v2s_features.csv', index=False)
    ...
    print(f"Improvement vs baseline V2-S: {best_auc - 0.9508:+.4f}")
    print(f"Results: {results_dir}")
    print(f"Submission: submission_v2s_features.csv")
    print("="*70 + "\n")
